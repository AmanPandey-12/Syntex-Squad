...
import {
  ...
  Save, Sparkles, ShieldCheck, Zap, Scan, Activity, FileText, ArrowRight, X, TrendingUp, MessageCircle, FileDown
} from 'lucide-react';
...
const DetectionPage = () => {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      if (data.isAgri === false) {
        setTimerActive(false);
        setError("Wrong image type. Please upload a crop or plant photo.");
        setLoading(false);
        return;
      }
  ...
  const shareToWhatsApp = async () => {
    ...
    // 1. Generate text for the WhatsApp message
    let text = `📄 *KRISHIAI BIOLOGICAL REPORT*\n`;
    text += `🌱 *CROP:* ${crop}\n`;
    text += `🔍 *DIAGNOSIS:* ${diag}\n`;
    text += `🏥 *HEALTH VITALITY:* ${score}%\n\n`;
    text += `Please find the attached professional diagnostic report for full details.`;
    
    // 2. Generate the HTML Report File (PDF-like Document)
    const reportHtml = generateHTMLReport({ ...result, nickname }, t, new Date(), language);
    const blob = new Blob([reportHtml], { type: 'text/html' });
    const reportFile = new File([blob], `KrishiAI_Report_${(nickname || crop).replace(/\s+/g, '_')}.html`, { type: 'text/html' });

    const shareData = {
      title: `${nickname || crop} Field Report`,
      text: text,
    };

    // Use Native Share API if available (best on mobile)
    try {
      if (navigator.share) {
        // Share the report file instead of the plant image
        if (navigator.canShare && navigator.canShare({ files: [reportFile] })) {
          shareData.files = [reportFile];
        }
        await navigator.share(shareData);
      } else {
        // Fallback for Browser/Desktop (Shared via Link/Text)
        const encodedText = encodeURIComponent(text + `\n\nView Image: ${result.imgUrl}`);
        window.open(`https://api.whatsapp.com/send?text=${encodedText}`, '_blank');
      }
    } catch (err) {
      console.warn("Share failed, falling back to URL", err);
      const encodedText = encodeURIComponent(text);
      window.open(`https://api.whatsapp.com/send?text=${encodedText}`, '_blank');
    }
  };
  ...
  const downloadReport = () => {
    ...
    a.href = url;
    a.download = `KrishiAI_Report_${(nickname || result.cropName).replace(/\s+/g, '_')}.html`;
    a.click();
                  ...
                  {!savedSuccess ? (
                    <div className="mt-12 pt-8 border-t border-slate-100">
                      ...
                      <div className="mb-8 p-6 bg-slate-50 border border-slate-200 rounded-2xl">
                        <label className="text-xs font-bold text-slate-900 uppercase tracking-wider block mb-3 px-1">Save as (Nickname)</label>
                        <div className="relative">
                          <FileText size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                          <input
                            type="text"
                            placeholder="e.g., North Field Tomato"
                            className="w-full pl-12 pr-6 py-3 bg-white border border-slate-200 focus:border-green-600 rounded-xl outline-none font-semibold text-slate-900 transition-all placeholder:text-slate-400 shadow-sm"
                            value={nickname}
                            onChange={e => setNickname(e.target.value)}
                          />
                        </div>
                        <p className="mt-2 text-[11px] text-slate-500 font-medium px-1">This name will be used to identify this scan in your inventory.</p>
                      </div>

                      {/* --- Simplified Actions --- */}
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <button
                          className="w-full py-4 bg-[#2e7d4f] text-white rounded-xl font-bold text-sm tracking-tight hover:bg-[#256640] transition-all flex items-center justify-center gap-2"
                        ...
                        <button
                          className="w-full py-4 bg-white border border-slate-200 text-slate-900 rounded-xl font-bold text-sm tracking-tight hover:bg-slate-50 transition-all flex items-center justify-center gap-2"
                        ...
                        <button
                          className="sm:col-span-2 w-full py-4 bg-slate-900 text-white rounded-xl font-bold text-sm tracking-tight hover:bg-black transition-all flex items-center justify-center gap-2 disabled:opacity-50"
